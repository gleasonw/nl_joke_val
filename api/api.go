...
func main() {
	...
	http.HandleFunc("/api/rolling_sum", func(w http.ResponseWriter, r *http.Request) {
		...
		grouping := r.URL.Query().Get("grouping")
		rows, err := db.Query(`
			SELECT SUM(count) OVER (ORDER BY created RANGE INTERVAL $1::interval PRECEDING),
				SUM(lol) OVER (ORDER BY created RANGE INTERVAL $1::interval PRECEDING),
				SUM(cereal) OVER (ORDER BY created RANGE INTERVAL $1::interval PRECEDING),
				SUM(monkas) OVER (ORDER BY created RANGE INTERVAL $1::interval PRECEDING),
				SUM(joel) OVER (ORDER BY created RANGE INTERVAL $1::interval PRECEDING),
				SUM(pogs) OVER (ORDER BY created RANGE INTERVAL $1::interval PRECEDING),
				SUM(huhs) OVER (ORDER BY created RANGE INTERVAL $1::interval PRECEDING),
